...
test("merges a historical OpenRouter Qwen entry into the primary Alibaba model", () => {
  const primary = {
    id: "aa-qwen3-8-max",
    name: "Qwen3.8 Max",
    slug: "qwen3-8-max",
  ...
  const result = dedupeOpenRouterVariantModels([primary, duplicate]);
  ...
  assert.equal(result[0].openrouter_weekly_rank, 112);
