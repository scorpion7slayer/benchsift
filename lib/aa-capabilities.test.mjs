...
test("prefers structured scripts and survives reordered or nested HTML", () => {
  const html = `
    <!doctype html>
    <html>
      <head>
        <meta
          content="Analysis of Example Labs&apos;s Atlas and comparison to other AI models."
          name="description"
        >
        <title><span>Ignored title wrapper</span></title>
      ...
      <body>
        <h1>
          Atlas <span>72B</span> Intelligence, Performance &amp; Price Analysis
        </h1>
        <p>Knowledge cutoff March 2025.</p>
        <p aria-hidden="true">"context_window_tokens": 1</p>
      </body>
    </html>
  `;

  const model = extractAAPartialModelData(html, "atlas-72b");
  const capabilities = extractAACapabilities(html);

  assert.equal(model.name, "Atlas 72B");
  assert.equal(model.release_date, "2026-07-20");
  assert.deepEqual(model.model_creator, {
    id: "example-labs",
    name: "Example Labs",
    slug: "example-labs",
  });
  assert.equal(capabilities.context_window_tokens, 256_000);
  assert.equal(capabilities.total_parameters_b, 72);
  assert.equal(capabilities.is_open_weights, true);
  assert.equal(capabilities.knowledge_cutoff, "March 2025");
});
